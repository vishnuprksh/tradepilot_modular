...
class Nitter:
    ...
    def _is_instance_encrypted(self):
        ...
        """
        page_over = False
        soup, page_over = self._get_page("/x")
    ...
    def _get_page(self, endpoint, max_retries=5):
        ...
        while keep_trying and (self.retry_count < max_retries):
            ...
            if r.ok:
                self.session_reset = False
                soup, page_over = self._check_error_page(soup)
                keep_trying = False
            else:
                soup, page_over = self._check_error_page(soup)
                if soup is None:
    ...
    def _search(
        ...
        soup, page_over = self._get_page(endpoint, max_retries)
        ...
        while keep_scraping:
            ...
            if (
                ...
                if soup.find_all("div", class_="show-more"):
                    ...
                    soup, page_over = self._get_page(next_page, max_retries)
                    if soup is None:
                        keep_scraping = False
                else:
                    keep_scraping = False
        return tweets, page_over
    ...
    def get_profile_info(self, username, max_retries=5, instance=None):
        ...
        soup, page_over = self._get_page(f"/{username}", max_retries)
        if soup is None:
            return None
